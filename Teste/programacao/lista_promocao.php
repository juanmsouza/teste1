...
<title>promocao de Eventos</title>
...
$sqln = mysql_query("SELECT * FROM promocao ORDER BY devento,mevento,aevento DESC");

$num = mysql_num_rows($sqln);

$total_paginas = ceil($num/$max);

$sqln = mysql_query("SELECT * FROM promocao where situacao='A' order by devento,mevento,aevento,hora,minuto DESC LIMIT ".$inicio.",".$max."");
...
<fieldset style="width:10%; margin:0 auto;border:0">
  ...
  print '<table width="300" border="0" align="right" cellpadding="0" cellspacing="0">';
...
$ano=date("Y");
 ...
 if($dif>0)
 {
 $valida = mysql_query("UPDATE promocao SET  situacao='I' WHERE id='$id'")or die(mysql_error());
 }
 if($dif<0)
 {
 $valida = mysql_query("UPDATE promocao SET  situacao='A' WHERE id='$id'")or die(mysql_error());
 }
  }
  $s = mysql_query("SELECT * FROM promocao ORDER BY devento,mevento,aevento DESC");

$n2 = mysql_num_rows($s);

$total_paginas = ceil($n2/$max);
$s= mysql_query("SELECT * FROM promocao where situacao='A' order by devento,mevento,aevento,hora,minuto DESC LIMIT ".$inicio.",".$max."");
$n4 = mysql_num_rows($s);
  for($i = 0; $i < $n4; $i++){
  ...
  $local = @mysql_result($s, $i, "conteudo");
  ...
  print '<tr>';
  print '<td align="right"><b><font color="#a7c03c">'.$devento.'/'.$mevento.'/'.$aevento.'</font></b></td>';
  print '</tr>';
  print '<tr>';
  print '<td align="right"><b><font color="b3b3b3">'.$evento.'</font></b></td>';
  print '</tr>';
  print '<tr>';
  print '<td align="right"><b><font color="b3b3b3">'.$local.'</font></b></td>';
  print '</tr>'; 
